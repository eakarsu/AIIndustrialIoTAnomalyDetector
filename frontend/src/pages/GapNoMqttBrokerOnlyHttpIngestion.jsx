...
// Auto-generated page for: No MQTT broker (only HTTP ingestion)

export default function GapNoMqttBrokerOnlyHttpIngestion() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-mqtt-broker-only-http-ingestion', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No MQTT broker (only HTTP ingestion)</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>No MQTT broker (only HTTP ingestion)</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run No MQTT broker (only HTTP ingestion)'}
        </button>
      </form>
